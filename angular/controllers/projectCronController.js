angular.module("projectCronController", []).controller("projectCronController",
        function ($scope, $rootScope, $http, companyService, $window, $document, $timeout) {
            var body = angular.element($document[0].body);
            var local_index = -1;
            $rootScope.projectServers = [];
            var id = $rootScope.pid;
            ...
            $rootScope.showCronJobModal = function (project_id,servers) {
                $rootScope.projectServers =[];
                for (var z in servers) {
                    if (servers[z].project_id == project_id) {
                        $rootScope.projectServers.push(servers[z].id);
                    }
                }
                if($rootScope.projectServers == undefined || $rootScope.projectServers==""){
                    alert("There are no servers for this project. Please create at least server before create cron.");
                    return false;
                }
                $rootScope.visible_cronjob = $rootScope.visible_cronjob ? false : true;
                $rootScope.jobName = "";
                ...
                if (value == "cronjob_cancel") {
                    ...
                    if (job_name == undefined || job_name.trim().length <= 0) {
                        ...
                        $http({
                            ...
                            data: {job_name: job_name, job_command: job_command.trim(), minute: minute, hour: hour, day_of_month: day_of_month, month: month, day_of_week: day_of_week,servers:$rootScope.projectServers},
                            headers: {"Content-Type": "application/json"}
                        }).success(function (data) {
                            if (data.success == 1) {
